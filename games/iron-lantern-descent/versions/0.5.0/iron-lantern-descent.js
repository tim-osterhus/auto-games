...
const IronLanternDescent = (() => {
  ...
  const GAME_DATA = {
    release: {
      version: "0.5.0",
      label: "v0.5.0 Cave Readability Repair",
      baseRelease: "v0.4.0 Echo Relay Network",
      summary: "Concept-led descent selection, compact survival HUD, Copper Iris onboarding, contextual mining, lantern, and banking actions, world labels, feedback meters, and hidden advanced ledgers publish on top of Echo Relay Network.",
    },
    assets: ASSET_PATHS,
    renderer: {
      name: "Three.js",
  ...
  function createInitialState(options = {}) {
    ...
    const state = {
      ...
      release: clone(GAME_DATA.release),
      player,
      movement: {
        collision: {
  ...
  function stageEvidenceState(state, options = {}) {
    ...
    const evidenceState = options.evidenceState || "stage-evidence";
    if (dom.root) {
      dom.root.dataset.evidenceState = evidenceState;
      dom.root.dataset.surface = showStart ? "expedition-start" : "descent";
    }
    if (dom["expedition-start"]) {
      dom["expedition-start"].hidden = !showStart;
      renderStartDepth(currentState.expeditionStart.selectedDepthId);
    }
    if (options.advancedLedger !== undefined) {
      toggleAdvancedLedger(Boolean(options.advancedLedger));
    ...
    return {
      evidenceState,
      surface: showStart ? "expedition-start" : "descent",
      release: currentState.release,
      phase: currentState.firstExpedition.phase,
      objective: currentState.run.objective,
      contextAction: clone(currentState.contextAction),
      banking: clone(currentState.banking),
      feedback: clone(currentState.feedback),
      cargo: clone(currentState.cargo),
      route: clone(currentState.route),
      lanterns: {
